  ...
  <div class="product-list">
    ...
    >
      <el-form
        ...
        size="small"
      >
        <el-form-item label="营销内容名称" prop="title">
        ...
        <el-form-item label="封面图" prop="imgUrl">
          ...
          <upload
            ...
            :fileList="imgUrlList"
          ></upload>
          <p>* 尺寸：320px*320px,大小控制在100kb以内</p>
        </el-form-item>
...
import {
  ...
  delLibraryById,
  sortLibrary
} from '@/api/xxxxxxxDashboard/libraries.js'
...
export default {
  ...
  methods: {
    ...
    getList() {
      getLibraries(this.filters).then(res => {
    ...
    publish(row) {
      ...
      this.$confirm(`Do you want to set this data ${type}?`, 'Tips', {
        ...
        updateLibrary(row.id, params).then(() => {
          this.getList()
        })
      })
    },
    ...
    submit(form) {
      this.$refs[form].validate(vaild => {
        if (vaild) {
          ...
          if (this.form.id) {
            updateLibrary(this.form.id, param).then(res => {
              console.log(res)
              this.dialogFormVisible = false
              this.getList()
            })
    ...
    delAction(row) {
      this.$confirm(`Would you like to delete the product?`, 'Tips', {
        ...
        delLibraryById(row.id).then(res => {
          console.log(res)
          this.getList()
        })
      })
    ...
    sort() {
      ...
      sortLibrary({ sorts: data }).then(res => {
        console.log(res)
        this.getList()
      })
    }
