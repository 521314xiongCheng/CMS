        ...
        <div class="list-item">
          <div class="list-item-content">
            <div class="list-item-header">
              <span class="release-time">更新于 {{ getNowDate() }}</span>
            ...
            <div class="item-btn">
              ...
              <span class="download" v-if="previewData.contentType === 1"
                >下载图片资源</span
              >
              <span class="download" v-else>下载视频资源</span>
            </div>
            <el-row
              :gutter="5"
            ...
            <div class="comment-content" v-if="previewData.commentText">
              ...
              <!-- <span class="unfold-box">... <s class="unfold">展开</s></span> -->
            </div>
            <div class="forword">
              <img
                class="icon"
                src="../../../images/library/forword.png"
                alt=""
              />
              <span>转发</span>
            </div>
            <!-- <div class="handle-btn">
                <el-button type="warning" plain size="small">推送</el-button>
              </div> -->
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getResourcesList(pageNo) {
      ...
      Http({
        ...
        if (this.uploadType === 1) {
          res['resources'] = res['resources'].filter(
            o => !oneOf(o.url, this.form.image)
          )
        }
        if (this.uploadType === 2) {
          res['resources'] = res['resources'].filter(
            o => o.url !== this.form.video.videoUrl
          )
        }
        if (this.uploadType === 3 || this.uploadType === 4) {
          res['resources'] = res['resources'].filter(
            o => o.url !== this.form.coverImageUrl
          )
        }
    ...
    search() {
      ...
      if (this.uploadType === 2) {
        this.findResourceForm.resourceType = 'video'
      } else {
    ...
    selectionChange(selection) {
      ...
      if (this.uploadType === 1) {
        let images = deepCopy(this.form.image)
        selection.forEach(o => {
          if (!oneOf(o.url, images)) {
            images.push(o.url)
          }
        })
        if (images.length > 9) {
    ...
    selectResource() {
      ...
      switch (this.uploadType) {
        case 1:
          ...
          break
        case 2:
          this.form.video.videoUrl = this.selection[0].url
          break
        case 3:
        case 4:
  ...
  .list-item-content {
    .list-item-header {
      ...
      .release-time {
        ...
        background: linear-gradient(
          270deg,
          rgba(255, 247, 245, 0) 0%,
          rgba(255, 247, 245, 1) 100%
        );
        border-radius: 0px 100px/2 100px/2 0px;
      }
      .popularity {
        display: flex;
        align-items: center;
    ...
    .item-btn {
      ...
      .copy-comment {
        ...
        border: 1px solid rgba(211, 204, 207, 1);
        color: #606266;
        font-size: 26px/2;
        line-height: 60px/2;
      }
