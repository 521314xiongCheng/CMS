    ...
    >
      ...
      <el-form-item label="可查阅等级限制" prop="careerLevel">
        ...
        <el-radio v-model="form.isLimitCareerLevel" :label="true"
          >职级限制</el-radio
        >
        <el-select v-model="form.careerLevel" v-if="form.isLimitCareerLevel">
          <el-option
        ...
        <span v-if="form.isLimitCareerLevel">以上可查看(包含)</span>
      </el-form-item>
  ...
  created() {
    ...
    if (this.$route.query.id) {
      getActivityDetail(this.$route.query.id).then(res => {
        ...
        if (res.careerLevel === -1) {
          res.careerLevel = null
        }
