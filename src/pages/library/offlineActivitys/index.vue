        ...
        <el-table-column label="操作" align="center" width="210" fixed="right">
          <template slot-scope="{ row, $index }">
            <el-button
              ...
              @click="toTop(row)"
              >{{
                row.isTop ? '已置顶' : row.isSort ? '置顶' : '未排序'
              }}</el-button
            >
...
import { getActivities, delActivity } from '@/api/library/offlineActivity.js'
...
export default {
  ...
  created() {
    let filters = storage.get('offlineFilters')
    console.log(filters)
    if (filters) {
  ...
  beforeRouteLeave(to, form, next) {
    if (to.path !== '/library/offlineActivitys/detail') {
      storage.set('offlineFilters', {
        searchName: '',
        isSearch: null,
        pageSize: 20,
        pageNo: 1
  ...
  methods: {
    getList() {
      getActivities(this.filters).then(res => {
    ...
    add() {
      this.$router.push({
        path: '/library/offlineActivitys/detail'
      })
    },
    toTop(row) {
      this.$confirm(`请问是否确认要将当前活动内容置顶?`, 'Tips', {
        ...
        topContent(row.activityId).then(res => {
          console.log(res)
          this.getList()
        })
      })
    ...
    editAction(row) {
      storage.set('offlineFilters', this.filters)
      this.$router.push({
        path: '/library/offlineActivitys/detail',
    ...
    changeSort() {
      ...
      this.tableData.data.forEach((o, idx) => {
        if (
          o.activityId === this.originalData[idx].activityId &&
