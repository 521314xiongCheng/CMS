...
export default {
  ...
  methods: {
    linkdialogSave() {
      const anchorNode = this.editor.selection.getSel().anchorNode
      ...
      if (anchorNode.localName == 'a') {
        ...
        anchorNode.parentNode.replaceChild(addelemtA, anchorNode)
      } else {
        let content = this.editor.selection.getContent()
        let linkTab = `<a href="${this.linkform.local}" data-rcapp="${this.linkform.rcapp}" data-rcmini="${this.linkform.rcmini}" data-local="${this.linkform.local}" >${content}</a>`
        this.editor.selection.setContent(linkTab)
      }
