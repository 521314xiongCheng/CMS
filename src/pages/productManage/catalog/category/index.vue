      ...
      <el-col :span="16" v-if="showDetail" class="item-detail">
        ...
        >
          ...
          <div id="customize">
            ...
            <div>
              <el-form-item
                v-for="item in detailForm.custom_parameters"
                :label="item.name"
                :key="item.id"
              >
                <el-input
...
import { getParameters } from '@/api/product/parameter.js'
...
export default {
  ...
  methods: {
    ...
    getCustoms() {
      getParameters({ page_size: 200, scene: 'CATALOG' })
        .then(res => {
          let list = res.results.list
          this.custom_parameters = list
        })
        .catch(() => {
          this.$notify({
            message: '获取自定义参数失败!',
    ...
    nodeClick(data, node) {
      ...
      getCategoryDetailById(data.id).then(res => {
        ...
        if (!res.result.custom_parameters) {
          res.result.custom_parameters = []
        } else {
          res.result.custom_parameters.forEach(o => {
            if (o.type === 3) {
              if (o.default_value) {
                o.imageList = [
                  {
    ...
    append() {
      ...
      this.share_image = []
      this.products = []
      this.detailForm.custom_parameters = this.custom_parameters
    },
    copy() {
        ...
        this.$refs[form].validate(vaild => {
          if (vaild) {
            ...
            let params = {
              ...
              custom_parameters: this[form].custom_parameters
            }
            if (!params.parent_id) {
            ...
            if (this[form].id) {
              ...
              // 添加分类
              params.custom_parameters = this[form].custom_parameters.map(
                c => ({
                  parameter_id: c.id,
